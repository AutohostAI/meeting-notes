...
def summarize_text(text: str, max_tokens: int = 2000) -> str:
    system = "\n".join([
        "You are a meeting assistant that loves to summarize meeting transcripts.",
        ...
        "Remember to keep the summary short and concise while retaining information about key descisions and action items.",
    ])
    openai.organization = os.getenv("OPENAI_ORG_ID")
    openai.api_key = os.getenv("OPENAI_API_KEY")
...
def summarize_long_text_in_chunks(text):
    ...
    if num_tokens_from_string(summary, "gpt-3.5-turbo") > 13000:
